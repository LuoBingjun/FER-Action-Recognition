...
from model.action_classifier import get_action_classifier
import json
...
act_table = {0: '打电话', 1: '打哈欠', 2: '抽烟', 3: '正常'}
...
class ActionClassifier:
    def __init__(self):
        self.model = get_action_classifier(checkpoint_path="ckpt/resnext50-32x4d/res11/model_best.pth.tar")
        self.table = act_table
        self.resizer = trans.Compose([
            trans.Scale(512),
    ...
    def detect(self, frame):
        ...
        frame = self.resizer(Image.fromarray(frame))
        frame = torch.unsqueeze(frame, 0)
        frame = frame.to(self.device)
        pred = self.model(frame).squeeze().detach()
        pred = F.softmax(pred, dim=0)
        prob, res = torch.max(pred, 0)
        res = res.item()
        prob = prob.item()
        pred = pred.cpu().numpy()
        return {"result": res, "probability": prob, "vector": pred}
        # TODO: temporal voting


class CameraSolver:
    def __init__(self, face_detector, classifier, act_classifier, fps=1):
        self.fps = fps
    ...
    def get_solved_frame(self):
        ...
        activities = None
        if self.act_classifier:
            activities = self.act_classifier.detect(frame)
        #self.classifier.draw(frame, expressions)
        return expressions, activities, frame
...
class CV2Visualizer:
    def __init__(self, label_table, act_label_table, color_table=None):
        self.label_table = label_table
        self.color_table = color_table
        self.act_label_table = act_label_table
        self.frame = None
        self.vector = None
        self.data = None

    def update(self, data, act_data, frame):
	...
	# activity
        if act_data:
            self.frame = self.draw_frame_act(act_data, self.frame)
    ...
    def draw_frame(self, data, frame):
        ...
        # frame = self.cv2ImgAddText(frame, "%s: %f"%(self.label_table[res], prob), x, y-50, color, 42)
        # cv2.putText(frame, "%s: %f" % (self.label_table[res], prob), (x, y - 10), cv2.FONT_HERSHEY_COMPLEX, 0.5, color, 1)

        for idx, label in enumerate(self.label_table):
            vec_str = "%s:%.2f"%(label, vec[idx])
            frame = self.cv2ImgAddText(frame, "%s"%vec_str, 20, 80 + idx * 50, (155, 0, 0) if idx == res else (0, 155, 0), 42)

        return frame

    def draw_frame_act(self, data, frame):
        res = data["result"]
        prob = data["probability"]
        pred = data["vector"]
        for i in range(4):
            frame = self.cv2ImgAddText(frame, "%s: %f" % (self.act_label_table[i], pred[i]), 900, 40 + i * 50, (155, 0, 0) if i == res else (0, 155, 0), 42)
            # cv2.putText(frame, "%s: %f" % (self.act_label_table[i], pred[i]), (900, 40 + i * 40), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 155) if i == res else (0, 155, 0), 2)
        return frame
...
class Visualizer:
    def __init__(self, label_table, act_label_table, window_size=20):
    ...
    def draw_frame_act(self, data, frame):
        res = data["result"]
        prob = data["probability"]
        cv2.putText(frame, "%s: %f" % (self.act_label_table[res], prob), (200, 200),
                    cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 155), 1)
        return frame
...
if __name__=="__main__":
    ...
    #cv2.imshow("", frame)
    #cv2.waitKey(0)
    vizor = Visualizer(label_table=classifier.express_table, act_label_table=action_classifier.table)
    while True:
